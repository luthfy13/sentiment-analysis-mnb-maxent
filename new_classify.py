...
def Training():
	...
	Length = len(LogCondProb)
	query = []
	query.append("insert into bobot (fitur,kelas,cond_prob) values")
	for sentimen in sentimenList:
		for fitur in featureList:
			query.append("('%s', '%s', %.17f)," % (fitur, sentimen, LogCondProb[fitur,sentimen]) )
...
def BacaModelKlasifikasi():
	...
	db = MySQLdb.connect("localhost","sent_a", "uppi","android_sa")
	cs = db.cursor()
	query = "SELECT fitur, kelas, cond_prob FROM test_bobot"
	try:
		cs.execute(query)
		rs = cs.fetchall()
		fitur = ''
	...
	for sentimen in sentimenList:
		...
		for fitur in kata:
			try:
				...
				bobot[sentimen] = float(bobot[sentimen]) + (math.log(float(alpha) / float(JumlahFitur[sentimen] + (V*alpha)), 2))
	temp = -9999
	hsl = ''
	for sentimen in sentimenList:
